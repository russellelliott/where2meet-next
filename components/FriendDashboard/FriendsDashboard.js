...
export default function FriendsDashboard({ onSignOut }) {
  ...
  function utcToLastContactDate(utcIso) {
    ...
    return `${year}-${month}-${day}`;
     ...
     /**
     * Helper to compute the max of two YYYY-MM-DD date strings and return as YYYY-MM-DD.
     */
  function maxDate(dateStrA, dateStrB) {
    if (!dateStrA) return dateStrB;
    if (!dateStrB) return dateStrA;
    return dateStrA >= dateStrB ? dateStrA : dateStrB;
    }
  ...
  const handleCompletePlannedHangout = useCallback(async (hangoutId, actualDate, attendeeIds) => {
    ...
    try {
      ...
      const localDateKey = utcToLastContactDate(actualDate);
          ...
          // Update lastContactDate for all attendees (friends collection), using max() to prevent decreasing
      for (const friendId of attendeeIds) {
        ...
        if (friend) {
          await updateFriend(user.uid, friendId, {
            contact: {
                  ...friend.contact,
              lastContactDate: maxDate(localDateKey, friend.contact?.lastContactDate),
                },
              });
            }
          }
   ...
   const handleCompleteHangout = useCallback(async (hangoutId, hangoutDatetime) => {
    ...
    try {
      ...
      const localDateKey = hangoutDatetime ? utcToLastContactDate(hangoutDatetime) : utcToLastContactDate(new Date().toISOString());
          ...
          // Update friend records with local date key, using max() to prevent decreasing
  ...
  return (
        <Box sx={{ p: { xs: 2, sm: 3 }, maxWidth: 1600, mx: 'auto', width: '100%' }}>
          {/* Error Alerts */}
         {error && (
