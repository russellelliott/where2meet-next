...
import { Search, Tag, Car, AlertTriangle, Trash2, Plus, UserX, MapPin, Pencil, Calendar, ArrowUpDown } from 'lucide-react';
...
function resolveFriendLocation(friendLoc, pois, cityCache) {
 ...
 export default function FriendList({
   ...
   const [selectedLogistics, setSelectedLogistics] = useState('all');
      // Sort state: 'none', 'name-asc', 'name-desc', 'contactAsc', 'contactDesc'
   ...
   const getStalenessCategory = (days) => {
     ...
     return 'overdue';
      ...
      // Sorting function
   const getSortedFriends = (friendList) => {
     if (sortConfig === 'none') return friendList;
     ...
     switch (sortConfig) {
       ...
       case 'contactAsc': {
           // Earliest date first; no-date entries last
           const compareDates = (fa, fb) => {
             const da = fa.contact?.lastContactDate || '';
             const db = fb.contact?.lastContactDate || '';
             if (!da && !db) return 0;
             if (!da) return 1;
             if (!db) return -1;
             return da.localeCompare(db); // ascending (older first)
           };
           return sorted.sort(compareDates);
         }
       case 'contactDesc': {
           // Most recent date first; no-date entries last
           const compareDates = (fa, fb) => {
             const da = fa.contact?.lastContactDate || '';
             const db = fb.contact?.lastContactDate || '';
             if (!da && !db) return 0;
   ...
   const filteredFriends = getSortedFriends(friends.filter((friend) => {
     const matchesSearch =
   ...
   const getSortLabel = (value) => {
     switch (value) {
       ...
       case 'contactAsc': return 'Contact Oldest';
       case 'contactDesc': return 'Contact Newest';
       default: return 'Sort';
     }
   };
   ...
   return (
       ...
       id="friend-list-card"
        >
          ...
          <Box sx={{ mb: 2.5 }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5, flexWrap: 'wrap', gap: 1 }}>
             ...
             Friends List
              ...
              <Chip
             label={`${filteredFriends.length} listed of ${friends.length} total`}
             size="small"
             sx={{
           ...
           id="friend-search-input"
            ...
            {/* Filters Row - Smaller Selectors */}
            <Box sx={{ display: 'flex', gap: 1, mt: 1.5 }}>
              {/* Contact Staleness - Smaller */}
              <Box sx={{ flex: 1, minWidth: 0 }}>
                <Typography variant="caption" sx={{ display: 'block', fontFamily: 'monospace', fontSize: '8px', textTransform: 'uppercase', fontWeight: 700, color: '#7D7B6D', mb: 0.5 }}>
               Contact Staleness
                </Typography>
                <TextField
               select
               ...
               sx={{
                 '& .MuiOutlinedInput-root': { minHeight: 28, fontSize: '11px' },
              ...
              <Box sx={{ flex: '0 0 auto', minWidth: 0 }}>
               ...
               Sort
                ...
                <FormControl fullWidth size="small" sx={{ '& .MuiOutlinedInput-root': { minHeight: 28, fontSize: '11px' } }}>
                  <Select
                   value={sortConfig}
                   ...
                   sx={{
                     fontSize: '11px',
                     '& .MuiSelect-select': { py: 0.5, fontSize: '11px' },
                   }}
                    >
                    <MenuItem value="none">Sort</MenuItem>
                    <MenuItem value="name-asc">Name A→Z</MenuItem>
                    <MenuItem value="name-desc">Name Z→A</MenuItem>
                    <MenuItem value="contactDesc">Contact Newest</MenuItem>
                    <MenuItem value="contactAsc">Contact Oldest</MenuItem>
                  </Select>
                </FormControl>
              </Box>
            </Box>
          ...
          <Box sx={{ flex: 1, overflowY: 'auto', display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)', md: 'repeat(3, 1fr)' }, gap: 1.5, pr: 0.5 }}>
               ...
               id={`friend-row-${friend.id}`}
                >
                   ...
                   <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                     {/* Left: Name & Location */}
                     <Box sx={{ flex: 1, minWidth: 0 }}>
                       <Typography variant="body2" sx={{ fontFamily: 'serif', fontWeight: 700, color: '#2D2D20', fontSize: '0.9rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                         {friend.name}
                       </Typography>
                       {/* Home POI Address */}
                       <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.25, flexWrap: 'wrap' }}>
                        ...
                        {/* Temporary Location - City first, date below on new line */}
                        ...
                        {locationData.tempDateRange && locationData.tempAddress && (
                          <Typography
                            variant="caption"
                            sx={{
                              display: 'block',
        ...
        <Box sx={{ display: 'flex', gap: 0.25, alignItems: 'center' }}>
                  ...
                  {isSelected && friend.logistics?.notes && (
                    <Typography variant="caption" sx={{ display: 'block', mt: 0.75, color: '#7D7B6D', fontStyle: 'italic', fontSize: '10px' }}>
            ...
            {filteredFriends.length === 0 && (
               <Box sx={{ textAlign: 'center', py: 6, px: 4, backgroundColor: '#FBFBF9', border: '1px dashed #E0DED7', borderRadius: 2, gridColumn: '1 / -1' }}>
