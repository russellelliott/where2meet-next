...
import { Calendar, Users, Clock, X, Flame, CheckCircle, Video, MapPin, ClipboardList } from 'lucide-react';
...
export default function HangoutScheduler({
  ...
  return (
     <div className="space-y-6">
       ...
       {isScheduling && (
         <div className="bg-[#FBFBF9] border border-[#EBE9E2] rounded-2xl p-5 shadow-xs">
                   ...
                   {friends.map((f) => {
                     ...
                     return (
                       <label key={f.id} className="flex items-center space-x-2 text-xs text-[#3D3D3D] hover:bg-[#FBFBF9] p-1 rounded cursor-pointer">
                         ...
                         />
                         <span>{f.name}</span>
                         <span className="text-[9px] text-[#7D7B6D] font-mono">({f.location?.city || 'Unknown'})</span>
                       </label>
                     );
                   })}
                 </div>
