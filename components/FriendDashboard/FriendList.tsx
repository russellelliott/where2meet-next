...
import { Search, MapPin, Tag, Mail, Car, AlertTriangle, Check, Trash2, Plus, UserX } from 'lucide-react';
...
interface FriendListProps {
  ...
  onRecordContact: (friendId: string) => void;
...
export default function FriendList({
  ...
  const [selectedStaleness, setSelectedStaleness] = useState<string>('all');
  ...
  const getStalenessCategory = (days: number) => {
  ...
  const cities = ['all', ...Array.from(new Set(friends.map(f => f.location.city)))];
  ...
  const getContactIcon = (primary: 'phone' | 'whatsapp' | 'discord' | 'email' | 'instagram') => {
    switch (primary) {
      case 'discord':
        return (
          <div className="flex items-center space-x-1.5" title="Discord">
            <FaDiscord size={16} color="#5865F2" />
            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#5865F2' }}>Discord</span>
          </div>
        );
      case 'whatsapp':
        return (
          <div className="flex items-center space-x-1.5" title="WhatsApp">
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#25D366' }}>WhatsApp</span>
          </div>
        );
      case 'instagram':
        return (
          <div className="flex items-center space-x-1.5" title="Instagram">
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#8a49a1' }}>Instagram</span>
          </div>
        );
      case 'email':
        return (
          <div className="flex items-center space-x-1.5" title="Email">
            <Mail className="w-4 h-4" style={{ color: '#666666' }} />
            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#666666' }}>Email</span>
          </div>
        );
      default:
        return (
          <div className="flex items-center space-x-1.5" title="Phone">
            ...
            <span className="text-[10px] font-bold uppercase tracking-wider" style={{ color: '#666666' }}>Phone</span>
          </div>
        );
    }
  };

  const getButtonStyles = (primary: 'phone' | 'whatsapp' | 'discord' | 'email' | 'instagram') => {
    switch (primary) {
      case 'discord':
        return {
          borderColor: '#5865F2',
      ...
      case 'email':
        return {
          borderColor: '#4a5d6e',
          color: '#4a5d6e',
          hoverClass: 'hover:bg-[#4a5d6e] hover:text-white',
        };
      default:
        return {
          borderColor: '#666666',
  ...
  return (
    <div id="friend-list-card" className="bg-white border border-[#EBE9E2] rounded-[24px] p-5 flex flex-col h-full shadow-xs">
        ...
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className={`px-3 py-1.5 text-xs font-bold rounded-xl flex items-center space-x-1 transition-all cursor-pointer shadow-xs ${
              isAddFormOpen 
                ? 'bg-[#F0EEE6] text-[#2D2D20] hover:bg-[#E0DED7]' 
                : 'bg-[#CC7A5C] hover:bg-[#b86649] text-white'
            }`}
      ...
      <div className="flex-1 overflow-y-auto space-y-3 max-h-[500px] pr-1" id="friend-items-container">
        <AnimatePresence>
              ...
              >
                ...
                <div className="mt-2 flex items-center justify-between">
                  <div className="flex flex-wrap gap-1">
                    {friend.tags.map(tag => (
                      <span key={tag} className="bg-[#F9F8F6] text-[#5A5A40] text-[9px] px-2 py-0.5 rounded-md font-medium border border-[#EBE9E2] flex items-center space-x-0.5">
                ...
                {isSelected && friend.logistics.notes && (
                  <motion.div
                    ...
                    className="mt-2.5 pt-2 border-t border-[#F0EEE6] text-[11px] text-[#7D7B6D] italic"
                  >
