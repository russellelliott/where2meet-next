...
/**
 * Normalize link field to be an array or null.
 * - null/undefined → null (empty state)
 * - string → [string] (single item array)
 * - array → as-is (if non-empty, otherwise null)
 * @param {null|string|string[]} link
 * @returns {null|string[]}
 */
export function normalizeLinkField(link) {
  if (link == null || (Array.isArray(link) && link.length === 0)) {
    return null;
  }
  ...
  if (Array.isArray(link)) {
    const filtered = link.filter(l => typeof l === 'string' && l.trim() !== '');
...
export async function createPoiFromPlaceResult({
  ...
  visibility = {},
   ...
   // Normalize link to array format
   const normalizedLink = normalizeLinkField(link);
